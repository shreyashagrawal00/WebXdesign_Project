        ...
        <form onSubmit={handleAuth} style={{ display: 'grid', gap: '1.25rem' }}>
            ...
            <>
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input id="name" name="name" type="text" aria-label="Full Name" placeholder="John Doe" required style={{ ...inputStyle, paddingLeft: '2.5rem' }} onChange={handleChange} />
                </div>
              </div>
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input id="phone" name="phone" type="text" aria-label="Phone Number" placeholder="+1 234 567 890" required style={{ ...inputStyle, paddingLeft: '2.5rem' }} onChange={handleChange} />
                </div>
              </div>
              <div>
                <label htmlFor="role" style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem', color: 'var(--text-muted)' }}>Account Type</label>
                <select id="role" name="role" aria-label="Account Type" style={inputStyle} onChange={handleChange} value={formData.role}>
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input id="email" name="email" type="email" aria-label="Email Address" placeholder="name@example.com" required style={{ ...inputStyle, paddingLeft: '2.5rem' }} onChange={handleChange} />
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input id="password" name="password" type="password" aria-label="Password" placeholder="••••••••" required style={{ ...inputStyle, paddingLeft: '2.5rem' }} onChange={handleChange} />
            </div>
          </div>
          ...
          <button
            ...
            disabled={isLoading}
            style={{ width: '100%', justifyContent: 'center', marginTop: '1rem', opacity: isLoading ? 0.7 : 1, cursor: isLoading ? 'not-allowed' : 'pointer' }}
          >
            {isLoading ? (isRegister ? 'Signing Up...' : 'Signing In...') : (isRegister ? 'Sign Up' : 'Sign In')}
