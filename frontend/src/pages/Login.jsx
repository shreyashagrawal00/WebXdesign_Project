    ...
    <div className="container animate-fade" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '80vh' }}>
      <div className="glass-card" style={{ width: '100%', maxWidth: '400px', padding: '2.5rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          {error && <p role="alert" aria-live="polite" style={{ color: 'var(--danger)', marginTop: '0.5rem', fontSize: '0.875rem' }}>{error}</p>}
        </div>
        ...
        <form onSubmit={handleAuth} style={{ display: 'grid', gap: '1.25rem' }}>
          {isRegister && (
            <>
              ...
              <div>
                ...
                <select id="role" name="role" style={inputStyle} onChange={handleChange} value={formData.role} disabled={isLoading}>
          ...
          <button type="submit" className="btn-primary" disabled={isLoading} style={{ width: '100%', justifyContent: 'center', marginTop: '1rem', opacity: isLoading ? 0.7 : 1, cursor: isLoading ? 'not-allowed' : 'pointer' }}>
            {isLoading ? (
              <>
                <Loader2 size={18} className="animate-spin" style={{ animation: 'logo-spin infinite 2s linear' }} />
